...
'''
DPG_Mixer_gwt :不采用动态图预测，采用的是gwt-offcial 学习到的图，作为动态图预测的消融实验

'''
class DPG_Mixer_gwt(nn.Module):
    ...
    def forward(self, x,adj,**kwargs):
        ...
        for i in range(self.pred_n):
            ...
            #     if mode == 'train':
            #         # 训练时使用真值作为下一次预测的输入
            #         input_x = torch.cat((input_x[:, :, :, -(L - self.iter_pred_len):], targets[:,:,:,i*self.iter_pred_len:(i+1)*self.iter_pred_len]), dim=-1)
            #         time=torch.cat((seq_time[:, :, :, -(L - self.iter_pred_len):], targets_time[:,:,:,i*self.iter_pred_len:(i+1)*self.iter_pred_len]), dim=-1)
            #     else:
            #         input_x = torch.cat((input_x[:,:,:,-(L-self.iter_pred_len):],iter_pred_output),dim=-1)
            #         time = torch.cat((seq_time[:, :, :, -(L - self.iter_pred_len):],targets_time[:, :, :, i * self.iter_pred_len:(i + 1) * self.iter_pred_len]),dim=-1)
            ...
            # TODO 测试adj问题
            adjacency_matrices = [adj_dynamic]
            # output dimension[B,D=1,N,L_iter]
            iter_pred_output = self.predictor(input_x_,adjacency_matrices).permute(0,3,1,2)
            iter_pred_list.append(iter_pred_output)
...
class XLSTM_dynamic_graph(nn.Module):
    def __init__(self,in_feature,d_model,save_path,num_nodes,pred_n,**kwargs):
        ...
        # self.key=GraphConvNet(d_model,d_model,dropout=0.3,support_len=1)
        # self.value = GraphConvNet(d_model,d_model,dropout=0.3,support_len=1)
    ...
    def forward(self,x,cell_past=None,normalize_past=None,m_past=None,**kwargs):
        ...
        for i in range(z.shape[-1]):
            ...
            if mode == 'test' and self.flag <12*self.pred_n:
