...
'''
DPG_Mixer_V3 不使用动态图，也不使用original_adj 仅仅使用预测头生成的season和trend的adj
'''
class DPG_Mixer_V3(nn.Module):
    def __init__(self, num_nodes, in_dim, out_dim,d_model,d_ff,pred_len,
        ...
        # TODO 输出头
        self.predictor = DPG_Mixer_Predictor(num_nodes, d_model, out_dim, d_model, d_ff, self.iter_pred_len,
                                             self.supports_len, dropout, kernel_size, blocks, layers, configs=args)
    ...
    def forward(self, x,adj,**kwargs):
        ...
        iter_pred_output,_,_ = self.predictor(input_x_,adjacency_matrices)
