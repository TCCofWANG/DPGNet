...
from model.DPG_base import DPG_Predictor

device=torch.device('cuda'if torch.cuda.is_available() else 'cpu')

'''
DPG_Net 采用动态图，但是输出头采用的是gwt_official的输出头，作为输出头的消融实验
...
class DPG_Net(nn.Module):
    def __init__(self, num_nodes, in_dim, out_dim,d_model,d_ff,pred_len,
        ...
        # TODO 输出头
        self.predictor = DPG_Predictor(num_nodes,d_model,out_dim,d_model,d_ff,self.iter_pred_len,dropout,kernel_size,blocks,layers)
    ...
    def forward(self, x,adj,**kwargs):
        ...
        # TODO 测试问题所在
        # adjacency_matrices = [original_adj, original_adj]

        iter_pred_output = self.predictor(input_x_,adjacency_matrices)
