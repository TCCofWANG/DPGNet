...
class MemoryGate(nn.Module):
    ...
    def __init__(self, hidden_size, num_nodes, mem_hid=32, input_dim=3, output_dim=1, memory_size=20,
        ...
        self.value =[nn.Parameter(torch.randn(hidden_size, mem_hid),requires_grad=True) for _ in range(3)]

        self.input_query = nn.Parameter(torch.randn(input_dim, mem_hid))

        self.We1 = nn.Parameter(torch.randn(num_nodes, memory_size))
        self.We2 = nn.Parameter(torch.randn(num_nodes, memory_size))
